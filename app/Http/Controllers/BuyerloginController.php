...
class BuyerloginController extends Controller
{
    public function vewfile($id)
    {
        ...
        $images = $query && $query->image ? explode(',', $query->image) : [];
    ...
    public function buyer_create(Request $request)
    {
        ...
        $buyer = DB::table('buyer')->insert([
            ...
            'name'     => $request->name,
            'phone'    => $request->phone,
            'password' => $request->password, // consider hashing later
    ...
    public function authenticate(Request $request)
    {
        ...
        if ($buyer) {
            ...
            return back()->with(['alert' => 'Incorrect password'])->withInput();
        }
        return back()->with(['alert' => 'No account found for this email'])->withInput();
    }
    ...
    public function buyer_dashboard(Request $request)
    {
        ...
        // NEW: categories list
        $category_data = DB::table('categories')->select('id', 'name')->orderBy('name')->get();
        ...
        $data = DB::table('qutation_form')
            ...
            ->select(
                ...
                'product.cat_id as product_cat_id', // kept if still stored
        ...
        // NOTE: You were filtering on qutation_form.cat_id (string/CSV?).
        // Keeping it as-is to avoid breaking current data, but category dropdown now comes from `categories`.
        if ($category) {
    ...
    public function accepted_list(Request $request, $data_id)
    {
        ...
        $data = DB::table('bidding_price')
            ...
            ->select(
                ...
                'bidding_price.product_id as bidding_price_product_id',
                'bidding_price.product_name as bidding_price_product_name',
                'bidding_price.created_at as date_time',
    ...
    public function seller_profile($id)
    {
        ...
        $email = $data12?->email;
        $reviews = $email ? DB::table('rating')->where('email', $email)->get() : collect();
        $rating = $reviews->average('star') ?: 0;
